...
module.exports.search = function (terms, done, ) {
    ...
    needle.request('get', HOST, `apiKey=trilogy&s=${query.s}`, function (err, res, movies) {
        if (err) {
            return done(err);
        }

        if (res.statusCode !== 200) {
          return console.log(res, res.statusCode);
...
module.exports.get = function (show, options, done) {
    ...
    needle.request('get', HOST, `apiKey=trilogy&plot=full&t=${query.t}`, function (err, res, movie) {
        if (err) {
          return console.log('assssasasas')
            return done(err);
        }
        ...
        if (res.statusCode !== 200) {
          // return console.log(res, res.statusCode);
            ...
            return console.log('asdfasdfsfasdget', HOST, query, res);
            // return done(new Error('status code: ' + res.statusCode));
        }
